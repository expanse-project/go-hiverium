...
var daoGenesisForkBlock = big.NewInt(5500000)
...
func DAOForkBlockNewChain(datadir string, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	if genesis != "" {
		...
		runGeth(t, "--datadir", datadir, "--networkid", "3", "init", json).WaitExit()
	} else {
		// Force chain initialization
		args := []string{"--port", "0", "--networkid", "3", "--maxpeers", "0", "--nodiscover", "--nat", "none", "--ipcdisable", "--datadir", datadir}
